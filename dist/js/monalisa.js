        ...
        _completeBoxImage: function(response) {
            ...
            for (var i = 0; i < response.length; i++) {
                ...
                // function readURL(input) {
                    ...
                    var aqui = reader.readAsDataURL(response[i]);
                // }
        ...
        init: function(context) {
            ...
            var defaults = {
                ...
                submitHandler: function(form) {
                    $(form).find('.loading').css('opacity', '1');
                    $(".Button").attr("disabled", true);
                    $.ajax({
                        type: "POST",
                        url: context.$el.attr('action'),
                        data: $(form).serialize(),
                        success: function () {
                            $(form).find('.Message').addClass('_success');
                            $(form).find('.loading').css('opacity', '0');
                            $(form).find('.Message').html("<h4>Mensagem enviada com sucesso!</h4>")
                                .hide()
                                .fadeIn(1500);
                            $(form).trigger('reset');
                        },
                        error: function () {
                            ...
                            $(form).find('.Message').html("<h4>Ocorreu um erro ao enviar sua mensagem.</h4>")
                                .hide()
                                .fadeIn(1500);
                        },
